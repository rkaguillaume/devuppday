class Reservation < ApplicationRecord
  ...
  scope :current_month_revenue, -> (user) {
    joins(:room)
    .where("rooms.user_id = ? AND reservations.updated_at >= ? AND reservations.status =  ?", user.id, 1.year.ago, 1)
    .order(updated_at: :asc)
  }

  scope :current_year_revenue, -> (user) {
    joins(:room)
    .where("rooms.user_id = ? AND reservations.updated_at >= ? AND reservations.status =  ?", user.id, 1.year.ago, 1)
    .order(updated_at: :asc)
  }
  ...
  def create_notification
    type = self.room.Instant? ? "Nouvelle réservation" : "Nouvelle requête"
